...
//ORIGINAL LINE: throw new ArgumentException();
	...
	{
		InitializeComponent();
		MainWindow::GetWindow(this)->Title += L" 1.4";
		...
		MessageBox::Show(L"Modifying your BIOS is dangerous and could cause irreversible damage to your GPU.\nUsing a modified BIOS may void your warranty.\nThe author will not be held accountable for your actions.", L"DISCLAIMER", MessageBoxButton::OK, MessageBoxImage::Warning);
	}

	void MainWindow::OpenFileDialog_Click(void *sender, RoutedEventArgs *e)
	{
		OpenFileDialog *openFileDialog = new OpenFileDialog();
		openFileDialog->Filter = L"BIOS (.rom)|*.rom|All Files (*.*)|*.*";
		...
		if (openFileDialog->ShowDialog() == true)
		{
			...
			{
				MessageBox::Show(L"This BIOS is less than the standard 512KB size.\nFlashing this BIOS may corrupt your graphics card.", L"WARNING", MessageBoxButton::OK, MessageBoxImage::Warning);
			}
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				...
				{
					msgSuported = MessageBox::Show(L"Unsupported DeviceID 0x" + deviceID + L" - Continue?", L"WARNING", MessageBoxButton::YesNo, MessageBoxImage::Warning);
				}
...
//C# TO C++ CONVERTER TODO TASK: This type of object initializer has no direct C++ equivalent:
	...
	void MainWindow::SaveFileDialog_Click(void *sender, RoutedEventArgs *e)
	{
		...
		SaveFileDialog->Title = L"Save As";
		SaveFileDialog->Filter = L"BIOS (*.rom)|*.rom";
		...
		if (SaveFileDialog->ShowDialog() == true)
		{
			...
			for (var i = 0; i < tableGPU::Items->Count; i++)
			{
				...
				auto mhz = static_cast<int>(int32->ConvertFromString((static_cast<TextBox*>(FindByName(L"MHZ", container)))->Text)) * 100;
				auto mv = static_cast<int>(int32->ConvertFromString((static_cast<TextBox*>(FindByName(L"MV", container)))->Text));
			...
			for (var i = 0; i < tableVRAM_TIMING::Items->Count; i++)
			{
				...
				auto value = (dynamic_cast<TextBox*>(FindByName(L"VALUE", container)))->Text;
				auto mhz = static_cast<int>(int32->ConvertFromString(name)) * 100;
